...
/-!
# Prékopa–Leindler on `Euc d`

The functional form of the Brunn–Minkowski inequality, in the shape needed by the
Wang–Xia assembly of the regularized closure route (`REGULARIZED_ROUTE.md`, lane L5):
if `f, g, h ≥ 0` satisfy `f x ^ (1 - t) * g y ^ t ≤ h ((1 - t) • x + t • y)` for all
`x, y`, then `(∫ f) ^ (1 - t) * (∫ g) ^ t ≤ ∫ h`.

Mathlib (v4.33.0) has neither Prékopa–Leindler nor Brunn–Minkowski.  The one-dimensional
case is proved here from scratch (level-set inclusion + 1-D Brunn–Minkowski + layer cake),
the `ι → ℝ` case follows by induction on the (finite) index type through Fubini, and the
`Euc d` case is transferred along the volume-preserving measurable equivalence
`MeasurableEquiv.toLp 2 (Fin d → ℝ)`.

Main results:

* `prekopaLeindler_1d` — the `ℝ≥0∞`-valued inequality on `ℝ`;
* `prekopaLeindler_pi` — the `ℝ≥0∞`-valued inequality on `ι → ℝ` (`ι` a `Fintype`);
* `prekopaLeindler_euc` — the `ℝ≥0∞`-valued inequality on `Euc d`, a.e.-measurable form;
* `prekopa_leindler` — the Bochner-integral form for nonnegative integrable functions;
* `one_le_integral_of_geometric_mean_le` — the normalized corollary `1 ≤ ∫ h`.

The 1-D proof is organized through named lemmas for the level-set inclusion, the
one-dimensional Brunn–Minkowski inequality, the resulting level-measure bound, and the
layer-cake integral assembly.  It is a port of `ref/statlean/PrekopaLeindler.lean`
(a sibling project, Mathlib v4.29.1) to the pinned Mathlib v4.33.0 of this repository.
-/
...
Haar measure). The transfer to `Euc d` is done in `prekopaLeindler_euc` via
...
This is the form transferred to `Euc d` in `prekopaLeindler_euc` below. -/
theorem prekopaLeindler_pi.{u}
    {ι : Type u} [Fintype ι]
    {f g h : (ι → ℝ) → ℝ≥0∞}
    (hf_meas : Measurable f) (hg_meas : Measurable g) (hh_meas : Measurable h)
    {t : ℝ} (ht_pos : 0 < t) (ht_lt : t < 1)
...
`Euc d = EuclideanSpace ℝ (Fin d)` is a type synonym for `Fin d → ℝ` carrying the Euclidean
norm; the identity map `MeasurableEquiv.toLp 2 (Fin d → ℝ)` is a measurable equivalence and
is volume preserving (`PiLp.volume_preserving_toLp`), and it is linear, so the
Prékopa–Leindler hypothesis and conclusion transfer verbatim. -/

section Euclidean

variable {d : ℕ}

/-- The identity map `(Fin d → ℝ) ≃ᵐ Euc d`, used to transfer the product Lebesgue measure
to the Euclidean volume. -/
private noncomputable def eucEquiv (d : ℕ) : (Fin d → ℝ) ≃ᵐ Euc d :=
  MeasurableEquiv.toLp 2 (Fin d → ℝ)

private theorem eucEquiv_affine (d : ℕ) (t : ℝ) (x y : Fin d → ℝ) :
    eucEquiv d (t • x + (1 - t) • y) = t • eucEquiv d x + (1 - t) • eucEquiv d y := rfl

private theorem measurePreserving_eucEquiv (d : ℕ) :
    MeasurePreserving (eucEquiv d) volume volume :=
  PiLp.volume_preserving_toLp (Fin d)

private theorem lintegral_eucEquiv (d : ℕ) (k : Euc d → ℝ≥0∞) :
    ∫⁻ z, k z = ∫⁻ x : Fin d → ℝ, k (eucEquiv d x) :=
  ((measurePreserving_eucEquiv d).lintegral_comp_emb
    (eucEquiv d).measurableEmbedding k).symm

/-- **Prékopa–Leindler on `Euc d`** (`ℝ≥0∞`-valued, measurable form). -/
theorem prekopaLeindler_euc_of_measurable {f g h : Euc d → ℝ≥0∞}
    (hf : Measurable f) (hg : Measurable g) (hh : Measurable h)
    {t : ℝ} (ht_pos : 0 < t) (ht_lt : t < 1)
    (h_le : ∀ x y : Euc d, f x ^ t * g y ^ (1 - t) ≤ h (t • x + (1 - t) • y)) :
    (∫⁻ x, f x) ^ t * (∫⁻ y, g y) ^ (1 - t) ≤ ∫⁻ z, h z := by
  have hfE : Measurable fun x : Fin d → ℝ => f (eucEquiv d x) :=
    hf.comp (eucEquiv d).measurable
  have hgE : Measurable fun x : Fin d → ℝ => g (eucEquiv d x) :=
    hg.comp (eucEquiv d).measurable
  have hhE : Measurable fun x : Fin d → ℝ => h (eucEquiv d x) :=
    hh.comp (eucEquiv d).measurable
  rw [lintegral_eucEquiv d f, lintegral_eucEquiv d g, lintegral_eucEquiv d h]
  refine prekopaLeindler_pi hfE hgE hhE ht_pos ht_lt ?_
  intro x y
  have hxy := h_le (eucEquiv d x) (eucEquiv d y)
  rw [← eucEquiv_affine d t x y] at hxy
  exact hxy

/-- **Prékopa–Leindler on `Euc d`** (`ℝ≥0∞`-valued, a.e.-measurable form).

Only a.e. measurability is assumed: `f` and `g` are replaced by measurable minorants
(set to `0` on a null set carrying the discrepancy with their measurable models) and `h`
by a measurable majorant (set to `⊤` there), which changes neither the hypothesis nor the
three integrals. -/
theorem prekopaLeindler_euc {f g h : Euc d → ℝ≥0∞}
    (hf : AEMeasurable f volume) (hg : AEMeasurable g volume) (hh : AEMeasurable h volume)
    {t : ℝ} (ht_pos : 0 < t) (ht_lt : t < 1)
    (h_le : ∀ x y : Euc d, f x ^ t * g y ^ (1 - t) ≤ h (t • x + (1 - t) • y)) :
    (∫⁻ x, f x) ^ t * (∫⁻ y, g y) ^ (1 - t) ≤ ∫⁻ z, h z := by
  classical
  obtain ⟨Nf, hNf_sub, hNf_meas, hNf_null⟩ :=
    exists_measurable_superset_of_null (ae_iff.mp hf.ae_eq_mk)
  obtain ⟨Ng, hNg_sub, hNg_meas, hNg_null⟩ :=
    exists_measurable_superset_of_null (ae_iff.mp hg.ae_eq_mk)
  obtain ⟨Nh, hNh_sub, hNh_meas, hNh_null⟩ :=
    exists_measurable_superset_of_null (ae_iff.mp hh.ae_eq_mk)
  -- Measurable modifications.
  set f' : Euc d → ℝ≥0∞ := Nf.piecewise (fun _ => 0) (hf.mk f) with hf'_def
  set g' : Euc d → ℝ≥0∞ := Ng.piecewise (fun _ => 0) (hg.mk g) with hg'_def
  set h' : Euc d → ℝ≥0∞ := Nh.piecewise (fun _ => ⊤) (hh.mk h) with hh'_def
  have hf'_meas : Measurable f' :=
    Measurable.piecewise hNf_meas measurable_const hf.measurable_mk
  have hg'_meas : Measurable g' :=
    Measurable.piecewise hNg_meas measurable_const hg.measurable_mk
  have hh'_meas : Measurable h' :=
    Measurable.piecewise hNh_meas measurable_const hh.measurable_mk
  -- Off the null sets the modifications agree with the original functions.
  have hf'_eq : ∀ x, x ∉ Nf → f' x = f x := by
    intro x hx
    have hfx : f x = hf.mk f x := by
      by_contra hne
      exact hx (hNf_sub hne)
    rw [hf'_def, Set.piecewise_eq_of_notMem _ _ _ hx, hfx]
  have hg'_eq : ∀ x, x ∉ Ng → g' x = g x := by
    intro x hx
    have hgx : g x = hg.mk g x := by
      by_contra hne
      exact hx (hNg_sub hne)
    rw [hg'_def, Set.piecewise_eq_of_notMem _ _ _ hx, hgx]
  have hh'_eq : ∀ x, x ∉ Nh → h' x = h x := by
    intro x hx
    have hhx : h x = hh.mk h x := by
      by_contra hne
      exact hx (hNh_sub hne)
    rw [hh'_def, Set.piecewise_eq_of_notMem _ _ _ hx, hhx]
  -- Pointwise comparisons.
  have hf'_le : ∀ x, f' x ≤ f x := by
    intro x
    by_cases hx : x ∈ Nf
    · rw [hf'_def, Set.piecewise_eq_of_mem _ _ _ hx]
      exact zero_le
    · exact le_of_eq (hf'_eq x hx)
  have hg'_le : ∀ x, g' x ≤ g x := by
    intro x
    by_cases hx : x ∈ Ng
    · rw [hg'_def, Set.piecewise_eq_of_mem _ _ _ hx]
      exact zero_le
    · exact le_of_eq (hg'_eq x hx)
  have hh'_ge : ∀ x, h x ≤ h' x := by
    intro x
    by_cases hx : x ∈ Nh
    · rw [hh'_def, Set.piecewise_eq_of_mem _ _ _ hx]
      exact le_top
    · exact le_of_eq (hh'_eq x hx).symm
  -- The modifications do not change the integrals.
  have hae_notMem : ∀ {N : Set (Euc d)}, volume N = 0 →
      ∀ᵐ x ∂(volume : Measure (Euc d)), x ∉ N := by
    intro N hN
    rw [ae_iff]
    simpa using hN
  have hf'_int : ∫⁻ x, f' x = ∫⁻ x, f x :=
    lintegral_congr_ae ((hae_notMem hNf_null).mono fun x hx => hf'_eq x hx)
  have hg'_int : ∫⁻ x, g' x = ∫⁻ x, g x :=
    lintegral_congr_ae ((hae_notMem hNg_null).mono fun x hx => hg'_eq x hx)
  have hh'_int : ∫⁻ x, h' x = ∫⁻ x, h x :=
    lintegral_congr_ae ((hae_notMem hNh_null).mono fun x hx => hh'_eq x hx)
  -- The Prékopa–Leindler hypothesis passes to the modifications.
  have h_le' : ∀ x y : Euc d, f' x ^ t * g' y ^ (1 - t) ≤ h' (t • x + (1 - t) • y) := by
    intro x y
    have h1 : f' x ^ t ≤ f x ^ t := ENNReal.rpow_le_rpow (hf'_le x) ht_pos.le
    have h2 : g' y ^ (1 - t) ≤ g y ^ (1 - t) :=
      ENNReal.rpow_le_rpow (hg'_le y) (by linarith)
    calc f' x ^ t * g' y ^ (1 - t) ≤ f x ^ t * g y ^ (1 - t) := mul_le_mul' h1 h2
      _ ≤ h (t • x + (1 - t) • y) := h_le x y
      _ ≤ h' (t • x + (1 - t) • y) := hh'_ge _
  rw [← hf'_int, ← hg'_int, ← hh'_int]
  exact prekopaLeindler_euc_of_measurable hf'_meas hg'_meas hh'_meas ht_pos ht_lt h_le'

/-! ## Bochner form -/

/-- **Prékopa–Leindler inequality on `Euc d`** for nonnegative integrable functions.

If `0 ≤ f, g, h` are integrable and
`f x ^ (1 - t) * g y ^ t ≤ h ((1 - t) • x + t • y)` for all `x, y`, then
`(∫ f) ^ (1 - t) * (∫ g) ^ t ≤ ∫ h`.

No measurability beyond integrability is assumed. -/
theorem prekopa_leindler {t : ℝ} (ht₀ : 0 < t) (ht₁ : t < 1) {f g h : Euc d → ℝ}
    (hf : ∀ x, 0 ≤ f x) (hg : ∀ x, 0 ≤ g x) (hh : ∀ x, 0 ≤ h x)
    (hfi : Integrable f) (hgi : Integrable g) (hhi : Integrable h)
    (hfgh : ∀ x y, f x ^ (1 - t) * g y ^ t ≤ h ((1 - t) • x + t • y)) :
    (∫ x, f x) ^ (1 - t) * (∫ x, g x) ^ t ≤ ∫ x, h x := by
  have h1t₀ : 0 < 1 - t := by linarith
  have h1t₁ : 1 - t < 1 := by linarith
  -- a.e. measurability of the `ℝ≥0∞`-valued versions
  have hFm : AEMeasurable (fun x => ENNReal.ofReal (f x)) volume :=
    ENNReal.measurable_ofReal.comp_aemeasurable hfi.aestronglyMeasurable.aemeasurable
  have hGm : AEMeasurable (fun x => ENNReal.ofReal (g x)) volume :=
    ENNReal.measurable_ofReal.comp_aemeasurable hgi.aestronglyMeasurable.aemeasurable
  have hHm : AEMeasurable (fun x => ENNReal.ofReal (h x)) volume :=
    ENNReal.measurable_ofReal.comp_aemeasurable hhi.aestronglyMeasurable.aemeasurable
  -- the hypothesis, in `ℝ≥0∞`
  have key : ∀ x y : Euc d,
      ENNReal.ofReal (f x) ^ (1 - t) * ENNReal.ofReal (g y) ^ (1 - (1 - t))
        ≤ ENNReal.ofReal (h ((1 - t) • x + (1 - (1 - t)) • y)) := by
    intro x y
    rw [show (1 : ℝ) - (1 - t) = t by ring]
    rw [ENNReal.ofReal_rpow_of_nonneg (hf x) h1t₀.le,
      ENNReal.ofReal_rpow_of_nonneg (hg y) ht₀.le,
      ← ENNReal.ofReal_mul (Real.rpow_nonneg (hf x) _)]
    exact ENNReal.ofReal_le_ofReal (hfgh x y)
  have hPL := prekopaLeindler_euc hFm hGm hHm h1t₀ h1t₁ key
  rw [show (1 : ℝ) - (1 - t) = t by ring] at hPL
  -- finiteness of the three integrals
  have hF_top : (∫⁻ x, ENNReal.ofReal (f x)) ≠ ⊤ :=
    ((hasFiniteIntegral_iff_ofReal (Filter.Eventually.of_forall hf)).mp
      hfi.hasFiniteIntegral).ne
  have hG_top : (∫⁻ x, ENNReal.ofReal (g x)) ≠ ⊤ :=
    ((hasFiniteIntegral_iff_ofReal (Filter.Eventually.of_forall hg)).mp
      hgi.hasFiniteIntegral).ne
  have hH_top : (∫⁻ x, ENNReal.ofReal (h x)) ≠ ⊤ :=
    ((hasFiniteIntegral_iff_ofReal (Filter.Eventually.of_forall hh)).mp
      hhi.hasFiniteIntegral).ne
  -- translate the three Bochner integrals
  ...
  exact ENNReal.toReal_mono hH_top hPL

/-- The normalized form of `prekopa_leindler` used by the Wang–Xia assembly: if `f` and `g`
are probability densities (nonnegative, integral one) and `h` dominates the weighted
geometric mean along the corresponding convex combinations, then `∫ h ≥ 1`. -/
theorem one_le_integral_of_geometric_mean_le {t : ℝ} (ht₀ : 0 < t) (ht₁ : t < 1)
    {f g h : Euc d → ℝ}
    (hf : ∀ x, 0 ≤ f x) (hg : ∀ x, 0 ≤ g x) (hh : ∀ x, 0 ≤ h x)
    (hfi : Integrable f) (hgi : Integrable g) (hhi : Integrable h)
    (hfgh : ∀ x y, f x ^ (1 - t) * g y ^ t ≤ h ((1 - t) • x + t • y))
    ...
    1 ≤ ∫ x, h x := by
  have := prekopa_leindler ht₀ ht₁ hf hg hh hfi hgi hhi hfgh
  rwa [hf1, hg1, Real.one_rpow, Real.one_rpow, one_mul] at this
